...
function CCR:CustomRoundedBox(rad, x, y, w, h, clr, tl, tr, bl, br)
	...
	if (tl) then
		local cache = self:GetDrawCache("roundedbox.tl")
		if cache then
			self:DrawCachedArc(cache, clr)
		else
			self:AddDrawCache("roundedbox.tl", CCR:DrawArc(x + rad, y + rad, 270, 90, rad, clr, rad))
		end
	else
		surface.SetDrawColor(clr)
	...
	if (tr) then
		CCR:DrawArc(x + w - rad, y + rad, 0, 90, rad, clr, rad)
	else
		surface.SetDrawColor(clr)
	...
	if (bl) then
		CCR:DrawArc(x + rad, y + h - rad, 180, 90, rad, clr, rad)
	else
		surface.SetDrawColor(clr)
	...
	if (br) then
		CCR:DrawArc(x + w - rad, y + h - rad, 90, 90, rad, clr, rad)
	else
		surface.SetDrawColor(clr)
