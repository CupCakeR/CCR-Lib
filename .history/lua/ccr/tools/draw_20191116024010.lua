...
function CCR:CustomRoundedBox(rad, x, y, w, h, clr, tl, tr, bl, br)
	...
	local cached = self:GetDrawCacheByKey()
	if cached then
		for key, value in pairs(cached) do
			self:DrawCachedArc(value, clr)

			local pre = "roundedbox."
			if key == (pre .. "tl") then
				tl = false
			elseif key == (pre .. "tr") then
				tr = false
			elseif key == (pre .. "br") then
				br = false
			elseif key == (pre .. "bl") then
				bl = false
			end
		end

		if !tl and !tr and !br and !br then return end
	end
