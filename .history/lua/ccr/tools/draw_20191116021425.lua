...
function CCR:CustomRoundedBox(rad, x, y, w, h, clr, tl, tr, bl, br)
	...
	if rad > 1 and CCR_CACHE_TARGET and CCR_CACHE_TARGET.tbl.CCR_CACHE then
		PrintTable(CCR_CACHE_TARGET.tbl.CCR_CACHE)
	end
	...
	if (br) then
		...
		if cache then
			...
			self:AddDrawCache(CCR:DrawArc(x + w - rad, y + h - rad, 90, 90, rad, clr, rad))
		end
	else
		surface.SetDrawColor(clr)
