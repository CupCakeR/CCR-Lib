...
if SERVER then
	util.AddNetworkString("CCR.OnPlayerFullyLoaded")

	net.Receive("CCR.OnPlayerFullyLoaded", function(len, p)
		CCR:Debounce("CCR.OnPlayerFullyLoaded." .. p:SteamID(), 0, function()
			if p.CCR_OnPlayerFullyLoaded then
				ErrorNoHalt(p:Nick() .. " -> already did init? Maybe sent net msg manually?\n")
				return
			end

			hook.Run("CCR.OnPlayerFullyLoaded", p)
			p.CCR_OnPlayerFullyLoaded = true
		end)
	end)
else
	hook.Add("SetupMove", "CCR.OnPlayerFullyLoaded", function()
		timer.Simple(15, function() // Im lazy
			net.Start("CCR.OnPlayerFullyLoaded")
			net.SendToServer()
		end)

		hook.Remove("SetupMove", "CCR.OnPlayerFullyLoaded")
	end)
end*/